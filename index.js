...
module.exports = function(configuration) {
  function send(req, extended) {
    ...
    if (configuration.country_code) {
      ...
      if (
        configuration.country_code !== "CA" &&
        !globals.hasOwnProperty(configuration.country_code + "_HOST")
      ) {
    ...
    return request(options)
      ...
      .then(res =>
        Array.isArray(res.response.receipt) ? res.response.receipt[0] : res.response.receipt
      );
  }
  ...
  var pay = function(args) {
    ...
    return $q.fcall(function() {
      ...
      return send(Object.assign(purchase, extended)).then(function(result) {
  ...
  var preauth = function(args) {
    ...
    var suffix = new Date().getTime() + "-" + Math.ceil(Math.random() * 10000);
    var order_id =
      args.order_id || cleanse(configuration.app_name, true) + "-PreAuth-" + suffix;
    var cust_id = args.cust_id || "customer-" + suffix;
    var dynamic_descriptor = args.description || args.dynamic_descriptor || "preauth";
    var extended = args.extended || {};
    var preauth = {
      type: "preauth",
      cust_id,
    ...
    return $q.fcall(function() {
      ...
      return send(Object.assign(preauth, extended)).then(function(result) {
        var fe = firstElement; //shorthand
        var code = fe(result.ResponseCode);
        var status = {
          msg: cleanse(fe(result.Message)),
          ...
          authCode: fe(result.AuthCode),
          timeout: fe(result.TimedOut, "true"),
          complete: fe(result.Complete),
          date: fe(result.TransDate),
          time: fe(result.TransTime),
          type: fe(result.TransType),
          custId: cust_id,
          orderId: order_id,
          transactionId: fe(result.TransID),
          transactionAmount: fe(result.TransAmount),
          messageId: fe(result.MessageId),
          avsCode: fe(result.AvsResultCode),
          cvdCode: fe(result.CvdResultCode)
        };
        var approved = !status.timeout && (code == "00" || code ? parseInt(code) < 50 : false);
        return $q.fcall(function() {
          if (approved) {
  ...
  var complete = function(args) {
    ...
    return $q.fcall(function() {
      ...
      return send(Object.assign(complete, extended)).then(function(result) {
        var fe = firstElement; //shorthand
        var code = fe(result.ResponseCode);
        var status = {
          msg: cleanse(fe(result.Message)),
  ...
  return { send, pay, preauth, complete };
};
